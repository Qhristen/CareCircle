...
GiftCircle turns fragmented group contributions into a transparent collective-support experience. This NestJS API supports the complete MVP journey: account creation, circle setup, gift wish lists, private invitations, guest/member contributions, Paystack payments, social updates, fulfillment tracking, and recipient confirmation.
...
The API defaults to `http://localhost:3000/api/v1`. Configure a Paystack secret key to initialize and verify real contributions. Set the Paystack webhook URL to `https://your-api.example/api/v1/webhooks/paystack`.
