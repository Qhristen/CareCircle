...
export function OrganizerDashboardRoute({ circleId }: { circleId: string }) {
  ...
  if (error || !dashboardQuery.data || !circleQuery.data || !contributionsQuery.data) {
    return <main className="mx-auto grid min-h-[65vh] max-w-xl place-items-center px-4 text-center"><div><Icon className="mx-auto text-primary" name="lock" size={34} /><h1 className="mt-4 text-2xl font-extrabold">{t("organizer.unavailable")}</h1><p className="mt-2 text-sm text-on-surface-variant">{getApiErrorMessage(error, t("organizer.unavailableBody"))}</p><Link className="mt-6 inline-flex rounded-full bg-primary px-5 py-2.5 text-sm font-bold text-white" href="/sign-in">{t("header.signIn")}</Link></div></main>;
  }
  ...
  const contributors: DashboardContributor[] = contributionsQuery.data.data.map((entry, index) => ({
    ...
    meta: `${new Date(entry.paidAt || entry.createdAt).toLocaleString(i18n.resolvedLanguage || "en", { dateStyle: "medium", timeStyle: "short" })} • ${entry.paymentReference}`,
    amount: entry.amountKobo / 100,
    allocation: entry.allocation?.name || t("organizer.generalFund"),
    rail: entry.paymentMethod === "bank_transfer" ? t("organizer.bankTransfer") : t("organizer.cardPaystack"),
    note: entry.note || t("organizer.noMessage"),
  ...
  const wishlist = managedItems.map((item) => ({
    ...
    emoji: item.emoji || "🎁",
    name: item.name,
    description: item.description || t("organizer.wishlistPriority"),
  ...
  const dashboardData: OrganizerDashboardData = {
    ...
    createdAt: managed.createdAt
      ? new Date(managed.createdAt).toLocaleDateString(i18n.resolvedLanguage || "en", {
          dateStyle: "medium",
        })
      : t("organizer.recentlyCreated"),
    recipientName,
    summary: t("organizer.summary", { occasion: t(`occasions.${occasion}`, { defaultValue: presentation.label }).toLowerCase(), name: recipientName }),
    ...
    bundleName: t("organizer.bundle", { name: recipientName, occasion: t(`occasions.${occasion}`, { defaultValue: presentation.label }) }),
    contributors,
    wishlist,
  };
